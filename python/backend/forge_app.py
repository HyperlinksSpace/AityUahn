"""Local forge API — ideas, backlogs, scaffold, tests, agents. Runs on the user's machine."""
...
def create_forge_app(forge: LForge | None = None, *, serve_ui: bool = True) -> FastAPI:
    ...
    @app.post("/api/forge")
    async def forge_project(body: ForgeRequest) -> dict[str, Any]:
    ...
    if serve_ui and STATIC_DIR.is_dir():
